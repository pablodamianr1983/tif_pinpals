...
class Messages:
    ...
    @classmethod
    def get_unique_users_for_channel(cls, channel_id):
        DatabaseConnection = current_app.config['DatabaseConnection']
        query = """SELECT DISTINCT user_id FROM Messages WHERE channel_id = %s"""
        params = (channel_id,)
        results = DatabaseConnection.fetch_all(query, params=params)

        # Aquí asumo que tu función fetch_all devuelve una lista de tuplas
        user_ids = [result[0] for result in results] if results else []
        return user_ids
